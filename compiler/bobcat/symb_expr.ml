...
module SymbExpr = struct
  ...
  let next_list_id = ref 0

  let mk_list len elts =
    incr next_list_id;
    Symb_list { id = !next_list_id; len; elts }
  let as_list = function Symb_list l -> Some l | _ -> None
  ...
  let mk_externalerror message =
    let open RuntimeError in
    Symb_error (make ExternalError message)
  ...
  let app_z3 (f : z3_expr -> z3_expr) = function
    ...
    | _ ->
      invalid_arg "[SymbExpr.app_z3] expected a z3 expression or incomplete"
  ...
  let app2_z3 (f : z3_expr -> z3_expr -> z3_expr) e1 e2 =
    ...
    | _ ->
      invalid_arg
        "[SymbExpr.app2_z3] expected two z3 expressions or an incomplete"
  ...
  (* TODO formatter *)
  let rec to_string ?(typed : bool = false) e =
    match e with
    | Symb_z3 s ->
    ...
    | Symb_list { len; elts; _ } ->
      let contents = String.concat "; " (List.map (to_string ~typed) elts) in
      "[" ^ contents ^ " | len=" ^ Z3.Expr.to_string len ^ "]"
    | Symb_none -> "None"
